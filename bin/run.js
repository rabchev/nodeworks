...
var startWorks = function (port, callback) {
    ...
    app = connect()
        ...
        .use(function (req, res) {
            if (req.url === "/") {
                res.writeHead(302, {Location: "/works/"});
                res.end();
            } else {
                res.writeHead(304);
                res.end("Not found");
    ...
    function exit(err) {
        if (err) {
            ...
            if (commander.start || commander.open) {
                determinePortAndStartWorks(callback);
            } else if (callback) {
                callback(err);
            } else {
                process.exit();
