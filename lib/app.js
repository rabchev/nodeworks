...
function startApp(debug, callback) {
    ...
    try {
        ...
        if ((app.scripts && app.scripts.start) || debug === "terminal") {
            setPort(debug, function (err, env) {
                if (err) {
                    ...
                    var cmd;
                    if (debug === "terminal") {
                        cmd = "node " + path.join(__dirname, "..", "node_modules", "web-terminal", "bin", "run");
                        debug = null;
                    } else {
                        cmd = app.scripts.start;
                    }
                    run(cmd, env, debug, callback);
                }
            });
        } else {
...
exports.terminalStart = function (callback) {
    "use strict";
    
    startApp("terminal", callback);
};
